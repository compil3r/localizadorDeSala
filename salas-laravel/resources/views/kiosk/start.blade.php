...
<head>
    ...
    <title>Start UniSenac - Primeiro dia</title>
    <link rel="stylesheet" href="{{ $assetBase }}/css/start.css">
...
<body>
    ...
    @if(!empty($meta['sim']))
        ...
        @endphp
        <form method="get" action="{{ url()->current() }}" class="sim-bar">
    ...
    <div class="app safe-area">
        <header class="hud">
            <div class="hud__brand">
                <h1 class="hud__title">START<span class="hud__caret">_</span></h1>
                <p class="hud__sub">UNI<span>SENAC</span> · aprender fazendo muda tudo</p>
            </div>
        </header>
        ...
        <main class="kiosk-main">
            <section id="screen-courses" class="screen screen--active screen--courses" aria-label="Lista de cursos">
                <div class="banner-quest" role="note">
                    <span class="banner-quest__icon" aria-hidden="true">►</span>
                    <span class="banner-quest__text">
                        <strong>1º SEMESTRE:</strong>
                        dirijam-se ao <b class="banner-quest__hi">AUDITÓRIO</b> — <b class="banner-quest__hi">9º ANDAR</b>
                    </span>
                    <span class="banner-quest__blink" aria-hidden="true">●</span>
                </div>

                <div class="screen-body">
                    <h2 class="screen-title">▸ SELECIONE SEU CURSO</h2>
                    <p class="screen-description">Toque no curso para ver as disciplinas e salas de hoje.</p>
            ...
            <section id="screen-course-detail" class="screen" aria-label="Disciplinas do curso selecionado">
                <div class="screen-body">
                    <div class="banner-quest banner-quest--mini" role="note">
                        <span class="banner-quest__icon" aria-hidden="true">►</span>
                        <span class="banner-quest__text"><strong>1º semestre:</strong> dirijam-se ao <b class="banner-quest__hi">AUDITÓRIO · 9º ANDAR</b></span>
                    </div>
                    ...
                    <div class="screen-actions">
                        <button id="btn-back-courses" class="btn-arcade btn-back" type="button">◄ VOLTAR AOS CURSOS</button>
                    </div>
                </div>
            </section>
        </main>
        ...
        (function () {
            ...
            function renderCourses() {
                ...
                if (cursos.length === 0) {
                    ...
                    dom.coursesGrid.innerHTML = '<div class="discipline-card discipline-card--empty"><h3 class="discipline-name">GAME OVER? Ainda não!</h3><p class="screen-description">' + msg + '</p></div>';
                    return;
                }
                ...
                cursos.forEach(function (curso) {
                    ...
                    const count = (curso.disciplinas || []).length;
                    btn.innerHTML = '<span class="course-card__badge">' + (curso.type === 'fic_hub' ? 'FIC' : 'CURSO') + '</span>'
                        + '<div class="course-card__code">' + (curso.codigoCurto || curso.nome || '') + '</div>'
                        + '<div class="course-card__name">' + (curso.nome || '\u00A0') + '</div>'
                        + '<div class="course-card__meta">' + count + ' ' + (count === 1 ? 'AULA' : 'AULAS') + ' <span class="course-card__go">JOGAR ►</span></div>';
            ...
            function appendDisciplineCard(container, d) {
                ...
                badge.className = 'badge-room' + (d.sala ? '' : ' badge-room--tbd');
            ...
            function renderDetail(frame) {
                ...
                if (frame.kind === 'grad_detail') {
                    ...
                    if (list.length === 0) {
                        ...
                        empty.className = 'discipline-card discipline-card--empty';
                        var msg = frame.isFic
